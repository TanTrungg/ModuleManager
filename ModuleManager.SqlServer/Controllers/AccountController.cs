    ...
    public async Task<IActionResult> Register(RegisterViewModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            await _emailService.SendEmail(
                ...
                $"Chào {user.UserName},<br>Bạn đã đăng ký thành công với tài khoản:<br>Email: {user.Email}<br>Mật khẩu: {model.Password}"
            );
            //var emailSent = await SendEmailAsync(user.Email, "Xác nhận đăng ký", $"Chào {user.UserName},"
            //    + $"\n\nBạn đã đăng ký thành công với tài khoản:\nEmail: {user.Email}\nMật khẩu: {model.Password}");

            //TempData["SuccessMessage"] = "Bạn đã đăng ký thành công.";
            //TempData["EmailMessage"] = emailSent ? "Email xác nhận đã được gửi." : "Không thể gửi email xác nhận.";
